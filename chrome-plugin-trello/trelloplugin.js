	...
	var appName 			= 'Agile SCRUM for Trello boards',
		...
		var _run = function()
		{
			...
			$('#board').find('.list-cards').each(function(){
				...
				currentListElement.find('.list-card').each(function(){
					...
					/* var el = currentCardElement.find('.badge .icon-checklist').html();
					 console.log(" el = "+el);
					sum += Number(el);
					console.log(" sum = "+sum);*/
			...
			var sum = 0;
			 $('.badge .icon-checklist').each(function() {
				 console.log(" $(this).next() = "+$(this).next().html())
				sum += eval($(this).next().html());
				console.log(" sum = "+sum)
			});
		...
		var _stringToColor = function( s )
		{
			...
			if(labels[label]){
				...
				console.log("je ne sais pas : its "+s);
			for ( ; i < len; i++ )
			{
